import { useState } from 'react';
import { Badge } from '@/components/ui/badge';

import { useRecordsCheckedDrill } from '../../api';
import { formatCount, formatCurrency, formatDate } from '../../helpers';
import { DashboardApiParams } from '../../types';
import { DrillPager } from './DrillPager';
import { DrillEmpty, DrillLoading } from './DrillStatus';

export const RecordsCheckedDrill = ({
  params,
  open,
}: {
  params: DashboardApiParams;
  open: boolean;
}) => {
  const [page, setPage] = useState(1);
  const { data, isLoading } = useRecordsCheckedDrill(params, page, open);

  if (isLoading) return <DrillLoading />;
  if (!data || data.data.length === 0)
    return <DrillEmpty label="No pre-booking records checked in this period." />;

  const totalPages = Math.max(Math.ceil(data.totalCount / data.pageSize), 1);

  return (
    <div className="flex-1 space-y-3 overflow-x-auto">
      <p className="text-xs text-muted-foreground">
        {formatCount(data.totalCount)} record{data.totalCount === 1 ? '' : 's'} checked
      </p>
      <table className="w-full min-w-[34rem] text-sm">
        <thead>
          <tr className="border-b text-left text-xs text-muted-foreground">
            <th className="py-2 font-medium">Checked</th>
            <th className="py-2 font-medium">Organization</th>
            <th className="py-2 font-medium">Period</th>
            <th className="py-2 text-right font-medium">Amount</th>
            <th className="py-2 text-right font-medium">Outcome</th>
          </tr>
        </thead>
        <tbody>
          {data.data.map((row) => (
            <tr key={row.id} className="border-b last:border-0">
              <td className="py-1.5 whitespace-nowrap text-muted-foreground">
                {formatDate(row.createdAt)}
              </td>
              <td className="py-1.5 max-w-[12rem] truncate">
                {row.organizationName}
              </td>
              <td className="py-1.5 whitespace-nowrap text-xs text-muted-foreground">
                {row.startDate && row.endDate
                  ? `${formatDate(row.startDate)} – ${formatDate(row.endDate)}`
                  : '—'}
              </td>
              <td className="py-1.5 text-right tabular-nums">
                {row.amount !== null && row.currency
                  ? formatCurrency(row.amount, row.currency)
                  : '—'}
              </td>
              <td className="py-1.5 text-right">
                <Badge variant={row.isSuccess ? 'secondary' : 'destructive'}>
                  {row.isSuccess ? 'clear' : 'blocked'}
                </Badge>
              </td>
            </tr>
          ))}
        </tbody>
      </table>
      {totalPages > 1 && (
        <DrillPager page={page} totalPages={totalPages} onChange={setPage} />
      )}
    </div>
  );
};
